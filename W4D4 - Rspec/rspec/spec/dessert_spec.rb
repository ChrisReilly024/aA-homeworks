...
describe Dessert do
  ...
  let(:cake) {Dessert.new('ice cream', 100, chef)}
  
  describe "#initialize" do
    it "sets a type" do
      expect(cake.type).to eq('ice cream')
    end
  ...
  it "raises an argument error when given a non-integer quantity" do
    expect {Dessert.new('ice cream', '2', chef)}.to raise_error(ArgumentError)
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      ...
      expect(cake.serve).to eq("Chef Tucker the Great Baker has made 100 ice creams!")
    end
  end
